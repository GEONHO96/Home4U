...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 19개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제·거절·archive 흐름 4** (백엔드 의존 X) / **role="alert" SR 영역 노출 2**. **전체 mock e2e 6 종이 axe-core wcag2a/aa critical/serious 0건 동시 검증** (재사용 가능한 `expectNoBlockingA11yViolations` fixture). archive sweep e2e 는 mockTransaction 의 `advanceTimeBy` controller 로 가상 시간 점프 — wall clock 대기 없이 즉시 검증. CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| e2e fixtures | `e2e/fixtures/backendMock.ts` 의 `mockBackend` / `injectFakeSession` / `mockChatRoom` / `mockTransaction` 헬퍼 + `e2e/fixtures/a11y.ts` 의 `expectNoBlockingA11yViolations` — 다른 e2e 가 백엔드 시드 없이 거래/결제/채팅 흐름까지 검증 + 동일 게이트로 axe wcag2a/aa scan. mockTransaction 은 approve/reject state machine + REJECTED **archive sweep timer 시뮬레이션** + buyer/seller/both view role + `advanceTimeBy(ms)` controller (가상 시간 점프 — wall clock 대기 없이 archive 흐름 검증) |
| 모바일 단위 테스트 | `home4u-app/{*.test.ts}` — vitest + tsx, RN/AsyncStorage/expo-notifications 의존을 vi.mock 으로 격리. **37개 케이스** — toastStore 디바운스 가드 4 + app.config.ts EAS_BUILD_PROFILE ATS 분기 4 + api.ts (formatPriceHuman / favorites / reviews / transactions / payments / chat / push) 21 + unreadStore (markRead / total / persist hydration race) 8. **`@vitest/coverage-v8` + 80% lines / 65% branches 임계값** 강제 + lcov 아티팩트 업로드. CI 에 별도 `mobile-test` 잡 |
