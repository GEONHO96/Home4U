...
| 모바일 앱 | Expo (Login/PropertyList/Detail/Favorites/Transactions/ChatList/ChatRoom). 거래/결제 흐름과 **JWT 인증 STOMP 실시간 채팅** (publish + subscribe, 실패 시 5초 폴링 fallback) + **읽음 영수증 · unread 뱃지 · OS 앱 아이콘 뱃지 카운트**. ChatRoom 진입 시 markRead → **zustand store** 가 ChatList unread 즉시 0 + OS 뱃지 sync. 앱 종료 상태에서도 **expo-background-fetch** 가 15분 간격으로 unread 합계를 갱신해 뱃지 유지. expo-notifications 로 Expo Push 토큰 등록 + 알림 탭 → ChatRoom/Transactions 자동 라우팅 |
| STOMP 인증 | `/ws-chat` STOMP CONNECT 단계에서 `StompJwtChannelInterceptor` 가 Authorization Bearer 헤더 검증 → cross-tenant 토큰 거부, 통과 시 Principal 이 SUBSCRIBE/SEND 까지 유지. `ChatStompController` 가 Principal 인자로 sender 를 식별 (payload userId 신뢰 X) |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **카테고리별 ✅/🚨 prefix + 임계값 미달 시 🔴 헤드라인** 으로 Slack/Discord 자동 알림 |
