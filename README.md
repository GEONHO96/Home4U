...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 18개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제·거절 흐름 3** (백엔드 의존 X) / **role="alert" SR 영역 노출 2** (chat/transactions fetch 실패 시 메시지가 aria-live 컨테이너로 들어가는지 행동 단위 검증). CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **카테고리별 ✅/🚨 prefix · 임계값 미달 시 🔴 헤드라인 · base branch (PR base 누락 시 main 최신) 점수와 ▲▼ 추세 비교** 후 Slack/Discord 자동 알림. base verify 스크립트는 OS-agnostic node `fs.readdirSync` 로 추출 + **windows-latest 회귀 잡**이 디렉토리 구분자/glob 차이 없이 동일하게 작동하는지 보호 |
| e2e fixtures | `e2e/fixtures/backendMock.ts` 의 `mockBackend` / `injectFakeSession` / `mockChatRoom` / `mockTransaction` 헬퍼 — 다른 e2e 가 백엔드 시드 없이 거래/결제/채팅 흐름까지 검증 가능 |
