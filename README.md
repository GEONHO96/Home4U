...
| 모바일 앱 | Expo (Login/PropertyList/Detail/Favorites/Transactions/ChatList/ChatRoom). 거래/결제 흐름과 **JWT 인증 STOMP 실시간 채팅** (publish + subscribe, 실패 시 5초 폴링 fallback) + **읽음 영수증 · unread 뱃지 · OS 앱 아이콘 뱃지 카운트**. ChatRoom 진입 시 markRead → 이벤트 버스로 ChatList 즉시 동기화. expo-notifications 로 Expo Push 토큰 등록 + 알림 탭 → ChatRoom/Transactions 자동 라우팅 |
| STOMP 인증 | `/ws-chat` STOMP CONNECT 단계에서 `StompJwtChannelInterceptor` 가 Authorization Bearer 헤더 검증 → cross-tenant 토큰 거부, 통과 시 Principal 이 SUBSCRIBE/SEND 까지 유지 |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **`SLACK_WEBHOOK_URL`/`DISCORD_WEBHOOK_URL` 시크릿 설정 시 점수 요약 자동 알림** |
