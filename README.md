...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 23개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 흐름 8** (채팅·결제·거절·archive·지연·unread 연동·self-sender unread X·dual clock) / **role="alert" SR 영역 노출 2**. mock e2e 9 종이 axe wcag2a/aa critical/serious 0건 동시 검증 + 위반 시 JSON attach. axe 결과 **2단계 캐시** (in-memory + sha1 디스크) + GitHub `actions/cache` 로 빌드 간 공유 (cached 16.7s vs cold 45s) + `clean:axe-cache` 스크립트로 mtime 30일+ 자동 GC. 시간 의존 흐름은 `advanceTimeBy` controller + `advanceTime` 헬퍼로 Node 가상 시간 + 브라우저 Playwright clock 동시 진행 — archive sweep / 지연 메시지 모두 wall clock 대기 0초 |
...
| 모바일 단위 테스트 | `home4u-app/{*.test.ts,*.test.tsx}` — vitest + tsx, RN/AsyncStorage/expo-notifications 의존을 vi.mock 으로 격리. **41개 케이스** — toastStore 4 + app.config 4 + api.ts 21 + unreadStore 8 + jsdom hook 4 (useUnreadHydrated 3 + useUnreadTotal selector 1). **`useUnreadTotal` 은 ChatListScreen 헤더 배지에 wire-up** — 합계가 변하면 markRead/setMany 직후 즉시 반영 (15s 폴링 의존 X). `@vitest/coverage-v8` + 80% lines/functions/statements + 70% branches 임계값 (현재 lines 95.53% / unreadStore 100%). CI `mobile-test` 잡 + PR 단계 coverage diff sticky-comment + **🔥 Hot files top 3 `<details>` collapsible** (코멘트 noise 절감) |
