...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 19개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제·거절·archive 흐름 4** (백엔드 의존 X, REJECTED sweep timer 포함) / **role="alert" SR 영역 노출 + 동시 axe-core wcag2a/aa scan 2** (에러 상태에서도 critical/serious 위반 0건). CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **카테고리별 ✅/🚨 prefix · 임계값 미달 시 🔴 헤드라인 · base branch (PR base 누락 시 main 최신) 점수와 ▲▼ 추세 비교** 후 Slack/Discord 자동 알림. base verify 스크립트는 OS-agnostic node `fs.readdirSync` 로 추출 + **ubuntu/windows/macos 3-OS matrix 회귀 잡**이 디렉토리 구분자/glob 차이 없이 동일하게 작동하는지 보호 |
| e2e fixtures | `e2e/fixtures/backendMock.ts` 의 `mockBackend` / `injectFakeSession` / `mockChatRoom` / `mockTransaction` 헬퍼 — 다른 e2e 가 백엔드 시드 없이 거래/결제/채팅 흐름까지 검증. mockTransaction 은 approve/reject state machine + REJECTED **archive sweep timer 시뮬레이션** (백엔드 admin 잡 모사) + buyer/seller/both view role 까지 처리 |
| 모바일 단위 테스트 | `home4u-app/{*.test.ts}` — vitest + tsx, RN 의존을 vi.mock 으로 격리. **toastStore 의 announce 디바운스 가드** (4 케이스) + **app.config.ts 의 EAS_BUILD_PROFILE ATS 분기** (4 케이스 — production 이 실수로 ATS 우회/cleartext 를 다시 켜지 않도록 정적 capture). CI 에 별도 `mobile-test` 잡 |
