...
| **Swagger · Tag 그룹** · Reports/Admin/Tenants 등 도메인 단위로 정리 | |
| ![swagger-tags](docs/screenshots/25-swagger-tags.png) | |
...
| 모바일 앱 | Expo (Login/PropertyList/Detail/Favorites/Transactions/ChatList/ChatRoom). 거래/결제 흐름과 **JWT 인증 STOMP 실시간 채팅** (publish + subscribe + 글로벌 toast 알림, 실패 시 5초 폴링 fallback) + **읽음 영수증 · unread 뱃지 · OS 앱 아이콘 뱃지 카운트 · BG 동기화 상태 표시**. ChatRoom 진입 시 markRead → **zustand store + AsyncStorage 영속 + `hasHydrated()` 가드**로 첫 프레임 깜빡임 제거. **글로벌 `useToast`** store 가 NavigationContainer 위 단일 마운트로 모든 화면(채팅·거래·결제) 에서 동일한 토스트 노출 + `AccessibilityInfo.announceForAccessibility` 로 VoiceOver/TalkBack 즉시 발화 (**동일 spoken 문구 1500ms 이내 디바운스 가드**로 OS 큐 잘림 방지). 앱 종료 상태에서도 **expo-background-fetch** 가 15분 간격으로 unread 합계를 갱신. `app.config.ts` 의 **3단계 EAS_BUILD_PROFILE 분기** (development / preview / production) 로 NSAppTransportSecurity·미디어 ATS·Android cleartext 를 단계별로 단단히 + `eas.json` + manual dispatch GitHub Actions 워크플로로 **EAS 빌드/TestFlight 업로드** 자동화 |
| STOMP 인증 | `/ws-chat` STOMP CONNECT 단계에서 `StompJwtChannelInterceptor` 가 Authorization Bearer 헤더 검증 → cross-tenant 토큰 거부, 통과 시 Principal 이 SUBSCRIBE/SEND 까지 유지. `ChatStompController` 가 Principal 인자로 sender 를 식별 (payload userId 신뢰 X). 핸들러 예외는 `@MessageExceptionHandler` 가 발신자 큐 (`/user/queue/chat-errors`) 로 STOMP ERROR JSON 통지 |
...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 25개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 흐름 10** (채팅·결제·거절·archive·지연·unread 연동·self-sender X·**per-message markRead**·**advanceTimeBy 음수 가드**·dual clock) / role="alert" SR 영역 노출 2. mock e2e 가 axe wcag2a/aa critical/serious 0건 동시 검증 + 위반 시 JSON attach. axe 결과 2단계 캐시 (in-memory + sha1 디스크) + GitHub `actions/cache` 로 빌드 간 공유 (cached 16.7s vs cold 45s) + **`.husky/post-merge` 가 머지 후 자동으로 `clean:axe-cache --days=14` 실행**. 시간 의존 흐름은 `advanceTimeBy` (음수 ms throw — 단방향 계약) + `advanceTime` 헬퍼로 Node 가상 시간 + 브라우저 Playwright clock 동시 진행 |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **카테고리별 ✅/🚨 prefix · 임계값 미달 시 🔴 헤드라인 · base branch (PR base 누락 시 main 최신) 점수와 ▲▼ 추세 비교** 후 Slack/Discord 자동 알림. base verify 스크립트는 OS-agnostic node `fs.readdirSync` 로 추출 + **ubuntu/windows OS matrix 회귀 잡** (PR 단계) + **macos-latest 는 push 시점에만 추가** (분당 8x 비용 가드) — 디렉토리 구분자/glob 차이 없이 동일하게 작동하는지 보호 |
| e2e fixtures | `e2e/fixtures/backendMock.ts` (`mockBackend` / `injectFakeSession` / `mockChatRoom` / `mockTransaction`) + `e2e/fixtures/a11y.ts` (`expectNoBlockingA11yViolations`) + `e2e/fixtures/time.ts` (`installVirtualClock` / `advanceTime`) — 다른 e2e 가 백엔드 시드 없이 거래/결제/채팅 흐름까지 검증 + 동일 게이트로 axe wcag2a/aa scan. mockTransaction · mockChatRoom 둘 다 `advanceTimeBy(ms)` controller 반환 → archive sweep / 지연 메시지 / 폴링 reconnect 흐름을 wall clock 대기 없이 검증. **`advanceTime` 헬퍼**가 Node 가상 시간 + 브라우저 Playwright clock 을 한 번에 advance — setInterval/setTimeout 기반 UI 흐름까지 가속. `mockChatRoom.delayedMessages` 가 `unread-count` 응답과 자동 연동 — 도착 시 +1, `markRead` 시 0 |
| 모바일 단위 테스트 | `home4u-app/{*.test.ts,*.test.tsx}` — vitest + tsx, RN/AsyncStorage/expo-notifications 의존을 vi.mock 으로 격리. **42개 케이스** — toastStore 4 + app.config 4 + api.ts 21 + unreadStore 8 + jsdom hook 5 (useUnreadHydrated 3 + useUnreadTotal selector 1 + **wire-up markRead 즉시 반영 1**). `useUnreadTotal` 은 ChatListScreen 헤더 배지에 wire-up — markRead 직후 즉시 N→N-k 반영 (15s 폴링 의존 X). **tsconfig `module: 'esnext'` + `tsc --noEmit` 가 mobile-test 잡에서 회귀 보호**. `@vitest/coverage-v8` + 80% lines/functions/statements + 70% branches (현재 lines 95.53%). CI `mobile-test` 잡 + PR 단계 coverage diff sticky-comment + 🔥 Hot files top 3 `<details>` collapsible |
