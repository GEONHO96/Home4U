...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 20개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제·거절·archive·지연 메시지 흐름 5** (백엔드 의존 X) / **role="alert" SR 영역 노출 2**. **전체 mock e2e 7 종이 axe-core wcag2a/aa critical/serious 0건 동시 검증** + **위반 발견 시 violation JSON 을 testInfo.attach 로 첨부** (HTML report + test-results 양쪽). `expectNoBlockingA11yViolations` 가 path + DOM 길이 키로 결과 캐싱 — 같은 페이지/상태 재진입 시 axe 호출 (~1s) 절감. archive sweep / 폴링 fallback 검증은 mockTransaction · mockChatRoom 의 `advanceTimeBy` controller 로 가상 시간 점프. CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| e2e fixtures | `e2e/fixtures/backendMock.ts` 의 `mockBackend` / `injectFakeSession` / `mockChatRoom` / `mockTransaction` 헬퍼 + `e2e/fixtures/a11y.ts` 의 `expectNoBlockingA11yViolations` — 다른 e2e 가 백엔드 시드 없이 거래/결제/채팅 흐름까지 검증 + 동일 게이트로 axe wcag2a/aa scan. **mockTransaction · mockChatRoom 둘 다 `advanceTimeBy(ms)` controller 반환** — 가상 시간 점프로 archive sweep / 지연 메시지 도착 / 폴링 reconnect 시뮬레이션을 wall clock 대기 없이 검증. mockChatRoom 의 `delayedMessages: [{ afterMs, ... }]` 옵션으로 30초 후 도착 메시지 같은 흐름도 1초 미만으로 e2e 화 |
| 모바일 단위 테스트 | `home4u-app/{*.test.ts,*.test.tsx}` — vitest + tsx, RN/AsyncStorage/expo-notifications 의존을 vi.mock 으로 격리. **40개 케이스** — toastStore 4 + app.config 4 + api.ts 21 + unreadStore 8 + **useUnreadHydrated React hook 본체 3 (jsdom env + @testing-library/react)**. **`@vitest/coverage-v8` + 80% lines/functions/statements + 70% branches 임계값** (현재 lines 95.49% / branches 73.52%). CI 에 별도 `mobile-test` 잡 + **PR 단계에서 base 와 비교한 coverage diff 를 sticky-pull-request-comment 로 노출** |
