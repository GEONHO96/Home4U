...
| 모바일 앱 | Expo (Login/PropertyList/Detail/Favorites/Transactions/ChatList/ChatRoom). 거래/결제 흐름과 **JWT 인증 STOMP 실시간 채팅** (publish + subscribe + 서버 ERROR 토스트/Alert, 실패 시 5초 폴링 fallback) + **읽음 영수증 · unread 뱃지 · OS 앱 아이콘 뱃지 카운트 · BG 동기화 상태 표시**. ChatRoom 진입 시 markRead → **zustand store + AsyncStorage 영속 + `hasHydrated()` 가드**로 첫 프레임 깜빡임 제거. 앱 종료 상태에서도 **expo-background-fetch** 가 15분 간격으로 unread 합계를 갱신해 뱃지 유지. **`app.config.ts`** 가 BG fetch / Notifications iOS Info.plist + Android permissions 를 prebuild 친화적으로 선언 |
...
| e2e fixtures | `e2e/fixtures/backendMock.ts` 의 `mockBackend(context, opts)` / `injectFakeSession(context)` 헬퍼로 다른 e2e 가 백엔드 의존 없이 화면을 검증할 수 있다 |
