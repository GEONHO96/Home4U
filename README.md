...
| 모바일 앱 | Expo (Login/PropertyList/Detail/Favorites/Transactions/ChatList/ChatRoom). 거래/결제 흐름과 **JWT 인증 STOMP 실시간 채팅** (publish + subscribe + 글로벌 toast 알림, 실패 시 5초 폴링 fallback) + **읽음 영수증 · unread 뱃지 · OS 앱 아이콘 뱃지 카운트 · BG 동기화 상태 표시**. ChatRoom 진입 시 markRead → **zustand store + AsyncStorage 영속 + `hasHydrated()` 가드**로 첫 프레임 깜빡임 제거. **글로벌 `useToast`** store 가 NavigationContainer 위 단일 마운트로 모든 화면(채팅·거래·결제) 에서 동일한 토스트 노출 + `AccessibilityInfo.announceForAccessibility` 로 VoiceOver/TalkBack 즉시 발화. 앱 종료 상태에서도 **expo-background-fetch** 가 15분 간격으로 unread 합계를 갱신. `app.config.ts` 의 **3단계 EAS_BUILD_PROFILE 분기** (development / preview / production) 로 NSAppTransportSecurity·미디어 ATS·Android cleartext 를 단계별로 단단히 |
...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 15개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제 흐름 2** (백엔드 의존 X). CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| Lighthouse CI | `lighthouserc.json` 기반 임계값 — performance ≥ 0.7 / a11y ≥ 0.9 / best-practices ≥ 0.85 / seo ≥ 0.8. CI 에 별도 `lighthouse` 잡으로 PR 단위 점수 추적 + **카테고리별 ✅/🚨 prefix · 임계값 미달 시 🔴 헤드라인 · base branch (PR base 누락 시 main 최신) 점수와 ▲▼ 추세 비교** 후 Slack/Discord 자동 알림 |
