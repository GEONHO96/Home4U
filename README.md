...
| Playwright e2e | `Home4u-frontend/e2e/*.e2e.ts` 22개 시나리오 — smoke / favorite / 거래·결제(실 백엔드) / a11y wcag 3 + skip-link 4 / **mock 기반 채팅·결제·거절·archive·지연 메시지·unread 연동·dual clock 흐름 7** (백엔드 의존 X) / **role="alert" SR 영역 노출 2**. **전체 mock e2e 9 종이 axe-core wcag2a/aa critical/serious 0건 동시 검증** + 위반 시 JSON attach. `expectNoBlockingA11yViolations` 가 **2단계 캐시** (in-memory + sha1 파일 디스크) + GitHub `actions/cache` 로 **빌드 간 공유** → cached run 16.7s (vs cold 45s). archive/폴링 fallback 검증은 `mockTransaction` · `mockChatRoom` 의 `advanceTimeBy` controller + `installVirtualClock` / `advanceTime` 헬퍼로 **Node 가상 시간 + 브라우저 Playwright clock 동시 진행**. CI 에 별도 `e2e` 잡으로 백엔드 백그라운드 + Vite + Chromium 가동 |
...
| 모바일 단위 테스트 | `home4u-app/{*.test.ts,*.test.tsx}` — vitest + tsx, RN/AsyncStorage/expo-notifications 의존을 vi.mock 으로 격리. **41개 케이스** — toastStore 4 + app.config 4 + api.ts 21 + unreadStore 8 + **jsdom hook 4** (useUnreadHydrated 3 + **useUnreadTotal selector 1 — RN-thin pattern 박제**). **`@vitest/coverage-v8` + 80% lines/functions/statements + 70% branches 임계값** (현재 lines 95.53% / branches 73.52%, unreadStore 100%). CI `mobile-test` 잡 + **PR 단계에서 base 와 비교한 coverage diff 를 sticky-pull-request-comment 로 노출** + **🔥 Hot files top 3** (file-level lines.pct |Δ|≥1pp, 신규 파일 우선) |
