 ...
 *
 * 흐름: PENDING 거래 거절 → 즉시 REJECTED 라벨 노출 → 가상 시간 advance → reload → 빈 목록.
 *
 * advanceTimeBy controller 로 wall clock 대기 없이 즉시 시간 점프 — wait time 1.7s → ~50ms.
 */
test('전체 mock — REJECTED 거래가 archive 윈도우 경과 후 목록에서 사라진다', async ({ page, context }) => {
  await mockBackend(context);
  // 실 wall clock 으로 wait 하지 않고 advanceTimeBy 로 점프하므로 archive ms 는 충분히 큰 값으로 설정
  ...
  // 가상 시간을 25시간 점프 — visibleState() filter 가 REJECTED + 1일 이상 경과한 항목 제거.
  // 실 wall clock 은 변하지 않아 e2e 가속 (page.waitForTimeout 불필요).
  tx.advanceTimeBy(25 * 60 * 60 * 1000);
  await page.reload();
